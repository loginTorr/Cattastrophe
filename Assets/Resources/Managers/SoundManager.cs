...
public class SoundFXManager : MonoBehaviour
{
    ...
    public static SoundFXManager GetInstance()
    {
        if (Instance == null)
        {
            ...
            {
                GameObject prefab = Resources.Load<GameObject>("Managers/SoundFXManager");
                Debug.Log(prefab);
    ...
    private Dictionary<int, AudioSource> activeLoopingSources = new Dictionary<int, AudioSource>();
        ...
        float volume = 1f, float pitch = 1f, float fadeInDuration = 0f)
    {
        ...
        activeLoopingSources[soundId] = source;
    ...
    public void StopLoopingSound(int soundId, float fadeOutDuration = 0f)
    {
        if (looping_audio.TryGetValue(soundId, out AudioSource source))
        {
            ...
            activeLoopingSources.Remove(soundId);
    ...
    public AudioSource GetAudioSourceFromId(int soundId)
    {
        if (activeLoopingSources.TryGetValue(soundId, out AudioSource source))
        {
            return source;
        }
        return null;
    ...
    public void FadeSoundById(int soundId, float targetVolume, float duration, bool stopAfterFade = false)
    {
        AudioSource source = GetAudioSourceFromId(soundId);
        if (source != null)
        {
            FadeSound(source, targetVolume, duration, stopAfterFade);
        }
    }
    #endregion
}